...
```bash
go get github.com/Bytonomics/multipay-india/multipay-go
```
...
Use [`client.Capabilities().Supports(provider, capability)`](./orchestration/capabilities.go) to check capability availability before attempting operations.
...
```go
import "github.com/Bytonomics/multipay-india/multipay-go/domain"
