...
help: ## Show this help
	@echo "MultiPay Adapter (github.com/Bytonomics/multipay-adapter) - Makefile targets:"
	@echo ""
...
unit-test-coverage: test-output-dir ## Run unit tests with coverage and threshold enforcement (fast, for pre-commit)
	...
	@rm -f /tmp/multipay-coverage.out
	@PACKAGES=$$(go list ./... | grep -v /testutil); \
	if [ -n "$$PACKAGES" ]; then \
		go test -v -race -cover -mod=mod -coverprofile=/tmp/multipay-coverage.out $$PACKAGES; \
	fi
	@if [ -f /tmp/multipay-coverage.out ]; then \
		go tool cover -html=/tmp/multipay-coverage.out -o /tmp/multipay-coverage.html; \
		echo "multipay unit test coverage report: /tmp/multipay-coverage.html"; \
	fi
	@echo "Checking unit test coverage..."
	@if [ -f /tmp/multipay-coverage.out ]; then \
		COVERAGE=$$(go tool cover -func=/tmp/multipay-coverage.out | grep total | awk '{print $$3}' | sed 's/%//'); \
		THRESHOLD=0; \
		echo "multipay unit test coverage: $${COVERAGE}% (target: $${THRESHOLD}%)"; \
		if [ -z "$$COVERAGE" ]; then \
