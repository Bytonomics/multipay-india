...
func listInstruments(ctx context.Context, adapter *Adapter, req *domain.GetInstrumentRequest) ([]*domain.Instrument, error) {
	if req == nil {
		return nil, fmt.Errorf("request is required: %w", domain.ErrInvalidRequest)
	}
...
func deleteInstrument(ctx context.Context, adapter *Adapter, req *domain.GetInstrumentRequest) error {
	if req == nil {
		return fmt.Errorf("request is required: %w", domain.ErrInvalidRequest)
	}

	if req.CustomerID == "" {
		return fmt.Errorf("CustomerID is required: %w", domain.ErrInvalidRequest)
	}

	if req.InstrumentID == "" {
		return fmt.Errorf("InstrumentID is required: %w", domain.ErrInvalidRequest)
	}
	...
	if err != nil {
		// Check if error is 404 instrument not found
		if isNotFoundError(err) {
			return fmt.Errorf("instrument %s not found: %w", req.InstrumentID, domain.ErrInstrumentNotFound)
		}
		return fmt.Errorf("failed to delete instrument on Cashfree: %w", domain.ErrProviderError)
	}
